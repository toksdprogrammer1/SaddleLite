...
public class CallEndpointServiceForFailed extends JobService {
    ...
    public boolean onStartJob(JobParameters jobParameters) {
        ...
        List<Cash> pendingCash = db.getPendingCash();
        if (pendingCash.size() == 0){
            ...
            editor.putBoolean("secondRunComplete", false);
            editor.commit();
            jobFinished(jobParameters, false);

            return false;
        }
        Log.d("Calling Db Size", pendingCash.size()+"");

        for (Cash cash : pendingCash){
            Log.d("about to send", cash.getId() + " " + cash.getAmount());
            //new connectNetplusEndpointTask(this, cash).execute(getEmail(), getPassword(),cash.getOrderNo(), cash.getAmount(),
             //       cash.getNarrative(), cash.getMerchantId());
